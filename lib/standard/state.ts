...
export default abstract class State {
  ...
  */
   ...
   */
  abstract async onEnter( event:StateEvent, preState:State ):Promise<any>;

  /**
   * @desc: leave the state. 即将进入下一状态, 故无法调用StateMechine.process_event触发状态切换.
   */
  abstract async onLeave( event:StateEvent, nextState:State ):Promise<void>;
   ...
   */
  abstract async onProcess( event:StateEvent, nextState:State ):Promise<{transition:boolean, ret?:any}>;
}